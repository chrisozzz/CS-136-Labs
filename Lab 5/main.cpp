...
class Inventory {
    private:
        ...
        */
        void sortCars(Car *pCars[], int searchField);
    ...
    public:
        ...
        */
        ...
        /*
        Print all valid car records in a formatted table from the array passed to the function
        Precondition: A pointer array of Cars is sorted
        Postcondition: Each car's data is printed to the terminal.
        */
        void printValidCars(Car *pCars[]);
...
void printSortMenu(){
    cout << "\n\nHow would you like to sort by (deceending): \n" 
            "1 ID\n"
            "2 Model\n"
            "3 Manufacturer\n"
            "4 Quantity\n"
...
void Inventory::sortCars(Car *pCars[], int searchField){
    
    if(searchField != EXIT_SORT_MENU){    
        for(int i = numValidCars-1; i>=0; i--){
            for(int j = 0; j < i; j++){
                
                Car *pCurrCar = pCars[j];
                Car *pNextCar = pCars[j+1];

                bool isNumberField = (searchField == SORT_PRICE || searchField == SORT_QUANTITY);
                bool currLessThanNext;

                if(isNumberField){
                    currLessThanNext = stod(pCurrCar->getDataField(searchField)) < stod(pNextCar->getDataField(searchField)); }
                
                else{
                    currLessThanNext = pCurrCar->getDataField(searchField) < pNextCar->getDataField(searchField); }
...
void Inventory::printValidCars(Car *pCars[]){
    printTableHeader();
...
void Inventory::promptSortCars(){
    ...
    Car *pParallelCars[numValidCars];

    for(int i = 0; i < numValidCars; i++){
        pParallelCars[i] = &validCars[i];
    }
    ...
    while(searchOptionField != EXIT_SORT_MENU){    
        ...
        switch(searchOptionField){
            case SORT_ID:
            case SORT_MODEL:
            case SORT_MANUFACTURER:
            case SORT_QUANTITY:
            case SORT_PRICE:
                sortCars(pParallelCars, searchOptionField);
                printValidCars(pParallelCars);
                break;
            case EXIT_SORT_MENU:
